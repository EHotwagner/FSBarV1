...
let writeInt32 (buf: byte[]) (offset: int) (value: int) =
    buf.[offset] <- byte (value &&& 0xFF)
    buf.[offset + 1] <- byte ((value >>> 8) &&& 0xFF)
    buf.[offset + 2] <- byte ((value >>> 16) &&& 0xFF)
    buf.[offset + 3] <- byte ((value >>> 24) &&& 0xFF)

let writeFloat32 (buf: byte[]) (offset: int) (value: float32) =
    let bytes = BitConverter.GetBytes(value)
    Array.blit bytes 0 buf offset 4

let writeUInt16 (buf: byte[]) (offset: int) (value: uint16) =
...
let buildMinimalSmf (version: int) (mapx: int) (mapy: int) : byte[] =
    let headerSize = 76
...
let avalancheAvailable () = File.Exists avalanchePath
