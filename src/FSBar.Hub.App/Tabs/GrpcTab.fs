...
module GrpcTab =
    ...
    let render
        ...
        let headerBlock =
            [ Scene.text heading (contentX + 8.0f) baseY 20.0f headingText
              Scene.text (sprintf "Endpoint: %s" endpointUrl) (contentX + 8.0f) (baseY + 24.0f) 15.0f bodyText ]
        match service with
        | None ->
            headerBlock @
            [ Scene.text
                "⚠ scripting service is not running — check BAR install detection."
                (contentX + 8.0f) (baseY + 50.0f) 14.0f bannerText ]
        | Some svc ->
            ...
            let rosterElems =
                [ yield Scene.rect (contentX + 8.0f) rosterTop panelW 28.0f panelBg
                  yield Scene.text "id" (contentX + 16.0f) (rosterTop + 20.0f) 13.0f dimText
                  yield Scene.text "label" (contentX + 100.0f) (rosterTop + 20.0f) 13.0f dimText
                  yield Scene.text "remote" (contentX + 260.0f) (rosterTop + 20.0f) 13.0f dimText
                  yield Scene.text "attached" (contentX + 500.0f) (rosterTop + 20.0f) 13.0f dimText
                  yield Scene.text "drops" (contentX + 620.0f) (rosterTop + 20.0f) 13.0f dimText
                  for i in 0 .. clients.Length - 1 do
                      ...
                      yield Scene.text (c.ClientId.ToString("N").Substring(0, 8))
                              (contentX + 16.0f) (rowY + 17.0f) 13.0f bodyText
                      yield Scene.text c.ClientLabel
                              (contentX + 100.0f) (rowY + 17.0f) 13.0f bodyText
                      yield Scene.text c.RemoteEndpoint
                              (contentX + 260.0f) (rowY + 17.0f) 13.0f bodyText
                      yield Scene.text attached
                              (contentX + 500.0f) (rowY + 17.0f) 13.0f bodyText
                      yield Scene.text (string c.CumulativeDroppedFrames)
                              (contentX + 620.0f) (rowY + 17.0f) 13.0f bodyText
                  if clients.IsEmpty then
                      yield Scene.text "(no clients connected — start one with `dotnet fsi src/FSBar.Hub/scripts/examples/03-launch-and-stream.fsx`)"
                              (contentX + 16.0f) (rosterTop + 52.0f) 13.0f dimText ]
            headerBlock @
            [ Scene.text summary (contentX + 8.0f) (baseY + 52.0f) 14.0f bodyText ]
            @ rosterElems
