...
let private tcpPair () : NetworkStream * Socket =
...
let private sendProxyMessage (server: Socket) (msg: ProxyMessage) =
...
let private recvAIMessage (server: Socket) : AIMessage =
...
let private frameMsg (n: uint32) : ProxyMessage =
...
let private callbackResponseMsg (reqId: uint32) : ProxyMessage =
...
let private readCallbackRequestId (server: Socket) : uint32 =
