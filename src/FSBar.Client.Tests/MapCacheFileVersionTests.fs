...
let private tinySupported () : MapCacheFile.SupportedMap =
    { MapName = "Synth Tiny"
...
let private rewriteIntField (path: string) (field: string) (newValue: int) =
...
let private rewriteStringField (path: string) (field: string) (newValue: string) =
    let text = File.ReadAllText path
    use doc = JsonDocument.Parse text
    use out = new MemoryStream()
    (
...
let private rewriteBaseCentre (path: string) (x: float32) (y: float32) (z: float32) =
    let text = File.ReadAllText path
    use doc = JsonDocument.Parse text
    use out = new MemoryStream()
    (
