...
let private clientSrcDir =
    Path.Combine(__SOURCE_DIRECTORY__, "..", "FSBar.Client") |> Path.GetFullPath
    ...
    if isUpdateMode () then
        ...
        for moduleName in fsiFiles do
            ...
            if not (File.Exists(baselinePath)) then
                let fsiContent = File.ReadAllText(Path.Combine(clientSrcDir, sprintf "%s.fsi" moduleName))
                File.WriteAllText(baselinePath, fsiContent)
...
let ``no_orphaned_baselines_exist`` () =
    if not (Directory.Exists(baselinesDir)) then () // No baselines dir = no orphans
    else
        let baselineFiles =
